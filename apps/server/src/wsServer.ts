...
> {
  ...
  const openCodeBridge = new OpenCodeBridge({
    onEvent: (event: OpenCodeEvent) => {
      void Effect.runPromise(pushBus.publishAll(WS_CHANNELS.opencodeEvent, event));
    },
  });
  ...
  const routeRequest = Effect.fnUntraced(function* (request: WebSocketRequest) {
    switch (request.body._tag) {
      ...
      case WS_METHODS.opencodeGetTodo: {
        const body = stripRequestTag(request.body);
        return yield* Effect.tryPromise({
          try: () => openCodeBridge.getTodo(body),
          catch: (cause) =>
            new RouteRequestError({
              message: `Failed to get OpenCode todo list: ${String(cause)}`,
            }),
        });
      }
